...
export default class VideoAssist extends DockTab {
	...
	constructor() {
		...
		getMediaDevies()
			.then(async (devices) => {
				...
				let error = false;
				const obsFeed = await navigator.mediaDevices
					.getUserMedia({
						video: {
	...
	render() {
		return html`
			<obs-dock-tab-section section-title="Histogram"> ${this.histogram} </obs-dock-tab-section>
			<obs-dock-tab-section section-title="Waveform"> ${this.waveform} </obs-dock-tab-section>
			<obs-dock-tab-section section-title="Vector Scope"> ${this.scope} </obs-dock-tab-section>
		`;
	}
}
